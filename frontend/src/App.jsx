...
function ChatPage({ user, onLogout }) {
  const [chatIds, setChatIds] = useState(() => loadChatIds(user.userId));
  const [activeChatId, setActiveChatId] = useState(() => loadChatIds(user.userId)[0]);
  ...
  async function handleNewChat() {
    try {
      ...
      const data = await res.json();
      const newChatId = Number(data.chat_id);

      const nextChatIds = [newChatId, ...chatIds];
      setChatIds(nextChatIds);
      saveChatIds(user.userId, nextChatIds);
...
function saveChatIds(userId, ids) {
  localStorage.setItem(`chats_${userId}`, JSON.stringify(ids));
}
